...
uint32_t __ctlba(BPB *block, uint32_t cluster, uint32_t first_data_sector) {
  return cluster_to_logical_block_addr(cluster, block, first_data_sector);
}

error_code __rbb(FILE *archive, BPB **block) {
  return read_boot_block(block, archive);
}
...
DEFINE_TEST(test_cluster_to_lba_1) {
        ...
        for (uint8_t i = 2; i < 10; ++i) {
            ck_assert_uint_eq(__ctlba(bpb, i, 1600), cluster_to_logical_block_addr(i, bpb, 1600));
        }
...
DEFINE_TEST(test_next_cluster_1) {
        STD_TEST();
        uint32_t next;
        get_cluster_chain_value(2, bpb, &next, archive);
...
DEFINE_TEST(test_check_name_1) {
        ...
        ck_assert(check_name(&entry, COERCE_STATIC_STR("ANAME")));
        ck_assert(!check_name(&entry, COERCE_STATIC_STR("NAME")));
        ck_assert(!check_name(&entry, COERCE_STATIC_STR("ANAM")));
...
DEFINE_TEST(test_check_name_2) {
        ...
        ck_assert(!check_name(&entry, COERCE_STATIC_STR("ANAME")));
        ck_assert(check_name(&entry, COERCE_STATIC_STR("ANAME.AAA")));
...
DEFINE_TEST(test_path_1) {
        ...
        if (split_up_path(0, path, &read) < 0 || NULL == read) {
            ck_abort();
        } else {
...
DEFINE_TEST(test_path_2) {
        ...
        if (split_up_path(1, path, &read) < 0 || NULL == read) {
            ck_abort();
        } else {
...
DEFINE_TEST(test_path_3) {
        ...
        if (split_up_path(2, path, &read) < 0 || NULL == read) {
            ck_abort();
        } else {
...
DEFINE_TEST(test_path_4) {
        ...
        if (split_up_path(3, path, &read) < 0 || NULL == read) {
            ck_abort();
        } else {
...
DEFINE_TEST(test_read_boot_block_1) {
        ...
        if (read_boot_block(&sbpb, fopen(ARCHIVE_PATH, "r")) < 0 || NULL == sbpb) {
            ck_abort();
        } else {
            ck_assert_uint_eq(as_uint16(bpb->BPB_BytesPerSector), as_uint16(sbpb->BPB_BytesPerSector));
            ck_assert_uint_eq(as_uint16(bpb->BPB_RsvdSectorCnt), as_uint16(sbpb->BPB_RsvdSectorCnt));
            ck_assert_uint_eq(as_uint16(bpb->BPB_RootEntCnt), as_uint16(sbpb->BPB_RootEntCnt));
            ck_assert_uint_eq(as_uint16(bpb->BPB_TotSector16), as_uint16(sbpb->BPB_TotSector16));
